...
//            MBProgressHUD.hide(for: self.view, animated: true)
    ...
    var uploadClient: UploadClient? {
        let api = API(host: globalDataConfig.baseUrl, version: Version(0, 60, 0))
        api?.userId = globalDataConfig.userID
        api?.authToken = globalDataConfig.userToken
        api?.language = "zh-CN"
        
        let client = api?.client(UploadClient.self)
        return client
    }

    func upload(_ file: FileUpload, fileName: String, description: String?) {
        uploadClient?.uploadMessage(roomId: globalDataConfig.roomID, data: file.data, filename: fileName, mimetype: file.type, description: description ?? "", progress: { [weak self] double in
            Log.debug("[REST][Progress]: \(double)")
